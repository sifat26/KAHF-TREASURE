...
import { adminOrderUrl } from '@/lib/whatsapp';
...
export default function OrderSuccessPage() {
  ...
  const adminWhatsAppUrl = useMemo(() => (order ? adminOrderUrl(order) : ''), [order]);
  const adminEmailUrl = useMemo(() => {
    ...
    const subject = encodeURIComponent(`New order ${order.trackingNumber}`);
    const body = encodeURIComponent(
      [
        'Assalamu Alaikum KAHF Treasure,',
        '',
  ...
  return (
    <div className='flex min-h-[60vh] items-center justify-center px-4 py-10'>
      <div className='w-full max-w-lg rounded-3xl border border-stone-200 bg-white p-8 text-center shadow-sm'>
        ...
        {order && (
          <div className='mt-3'>
            <ButtonLink href={adminWhatsAppUrl} external variant='secondary' size='lg' full>
              WhatsApp-এ admin-কে পাঠান
            </ButtonLink>
            <ButtonLink href={adminEmailUrl} external variant='outline' size='lg' full className='mt-3'>
              Email-এ admin-কে পাঠান
            </ButtonLink>
            <p className='mt-2 text-xs text-stone-500'>
              চাইলে WhatsApp বা email দিয়ে অর্ডার সারাংশ admin-এর কাছে পাঠাতে পারেন।
            </p>
          </div>
        )}
      </div>
