                ...
                {products.map((product) => {
                  ...
                  return (
                    <tr key={product._id} className='border-b border-stone-50 hover:bg-amber-50/20 transition-colors'>
                          ...
                          <div className='min-w-0'>
                            ...
                            <div className='mt-0.5 flex gap-1'>
                              {product.isFeatured && (
                                <span
                                  title='ফিচার্ড'
                                  className='inline-flex h-4 w-4 items-center justify-center rounded bg-amber-100'
                                >
                              ...
                              {product.newArrival && (
                                <span
                                  title='নতুন'
                                  className='inline-flex h-4 w-4 items-center justify-center rounded bg-blue-100'
                                >
                              ...
                              {product.isOnOffer && (
                                <span
                                  title='অফারে'
                                  className='inline-flex h-4 w-4 items-center justify-center rounded bg-red-100'
                                >
                      ...
                      <td className='p-3'>
                        <span
                          className={`font-semibold ${isOutOfStock ? 'text-red-600' : isLowStock ? 'text-amber-600' : 'text-emerald-600'}`}
                        >
                          {totalStock}
                        </span>
                      ...
                      <td className='p-3'>
                        <div className='flex flex-wrap gap-1 max-w-[120px]'>
                          {(product.tags || []).slice(0, 2).map((tag) => (
                            <span
                              key={tag}
                              className='flex items-center gap-0.5 rounded bg-stone-100 px-1.5 py-0.5 text-[10px] text-stone-500'
                            >
                      ...
                      <td className='p-3'>
                        <span
                          className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${product.isActive ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-stone-50 text-stone-500 border-stone-200'}`}
                        >
...
function ProductModal({
  ...
  const [form, setForm] = useState({
    ...
    categoryId:
      typeof product?.categoryId === 'object' ? (product?.categoryId as any)?._id || '' : product?.categoryId || '',
        ...
        <div className='p-6 space-y-5'>
          ...
          <div className='grid gap-3 sm:grid-cols-2'>
            ...
            <FormInput
              ...
              placeholder='যেমন: KAHF, OUD House'
            />
          </div>
          ...
          <div className='grid gap-3 sm:grid-cols-2'>
            <FormInput
              ...
              placeholder='bestseller, fresh, men'
            />
            <div className='grid grid-cols-2 gap-3'>
              <FormInput
                label='সর্টিং ক্রম'
                type='number'
                value={String(form.productOrder)}
                onChange={(v) => setForm({ ...form, productOrder: Number(v) })}
              />
              <FormInput
                label='কম স্টক সীমা'
                type='number'
                value={String(form.lowStockThreshold)}
                onChange={(v) => setForm({ ...form, lowStockThreshold: Number(v) })}
              />
            </div>
          </div>
            ...
            <div className='flex flex-wrap gap-3'>
              {imageUrls.map((url, i) => (
                <div key={i} className='relative h-20 w-20 group'>
                  ...
                  {/* Primary badge */}
                  {i === 0 && (
                    <span className='absolute bottom-0.5 left-0.5 rounded bg-amber-600/90 px-1 py-0.5 text-[8px] font-bold text-white'>
                      মূল
                    </span>
                  )}
                  <button
              ...
              {/* Upload button */}
              <label
                className={`flex h-20 w-20 cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed transition ${uploading ? 'border-amber-300 bg-amber-50' : 'border-stone-300 text-stone-400 hover:border-amber-400 hover:text-amber-600 hover:bg-amber-50'}`}
              >
                {uploading ? (
                  <>
          ...
          <div className='grid grid-cols-2 gap-2 sm:grid-cols-4 rounded-xl bg-stone-50 p-3'>
            {[
              ...
              <label
                key={f.key}
                className='flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 text-sm text-stone-600 hover:bg-white transition'
              >
                <input
                  type='checkbox'
