...
export function ProductCard({ product, className }: { product: Product; className?: string }) {
  ...
  return (
    <article
      className={cn(
        'group relative flex min-w-0 flex-col overflow-hidden rounded-[20px] border border-[var(--color-border)] bg-[var(--color-card)] shadow-sm transition-all duration-500 hover:-translate-y-1.5 hover:border-[var(--color-gold)]/50 hover:shadow-2xl hover:shadow-[var(--color-gold)]/10 select-none sm:rounded-2xl',
        className,
      )}
    >
      {/* Media Box */}
      ...
      <div className='flex flex-1 flex-col justify-between bg-[var(--color-background)] p-3.5 sm:p-5'>
        <div>
          ...
          <h3 className='font-serif text-sm font-bold leading-tight text-[var(--color-text-primary)] transition-colors group-hover:text-[var(--color-gold)] line-clamp-2 sm:text-lg sm:line-clamp-1'>
            <Link href={href}>{productLabel(product)}</Link>
          </h3>
